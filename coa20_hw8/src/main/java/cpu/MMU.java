...
public class MMU {
    ...
    public char[] read(String logicAddr, int length) {
        ...
        if (!Memory.PAGE) {  // 实模式或分段模式
            if (!Memory.SEGMENT) {  // 这种情况不需要cache和tlb
                ...
                if (!memory.isValidSegDes(Index)) {
                    char[] base = memory.seg_load(Index).toCharArray();
                    linearAddr = add(base, logicAddr.substring(16));
                } else {
            ...
            int offset = -1;
            int endvPageNo = -1;
            startvPageNo = Integer.parseInt(t.binaryToInt("0" + logicAddr.substring(16, 36)));
            offset = Integer.parseInt(t.binaryToInt("0" + logicAddr.substring(36)));
            endvPageNo = startvPageNo + (length - (Memory.PAGE_SIZE_B - offset)) / Memory.PAGE_SIZE_B;
            if ((length - (Memory.PAGE_SIZE_B - offset)) % Memory.PAGE_SIZE_B != 0) {
                endvPageNo += 1;
            }
            ...
            for (int i = startvPageNo; i <= endvPageNo; i++) {
                ...
                // Assume it true
                /**------------------------------------------------------------------------*/
                int tlbRowNO = tlb.isMatch(i);
                if (tlbRowNO != -1) {  // 此时主存中已经加载了该页的数据
                    pageAddr = String.valueOf(tlb.getFrameOfPage(tlbRowNO));
    ...
    private int getSegIndex(String logicAddr) {
        //TODO TRUE
        return Integer.parseInt("0" + t.binaryToInt(logicAddr.substring(0, 13)));
    }
    ...
    private String toRealLinearAddr(String logicAddr) {
        //TODO TRUE
        return "000000000000" + logicAddr.substring(0, 4) + logicAddr.substring(32);
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        //TODO linearAddr = SegBase + offset TRUE
        char[] base = Memory.getMemory().getBaseOfSegDes(getSegIndex(logicAddr));
