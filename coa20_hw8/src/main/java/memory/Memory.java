...
public class Memory implements MemoryInterface {
    ...
    private static char[] memory = new char[MEM_SIZE_B];  // memory 存储数据的地方
    ...
    public String seg_load(int segIndex) {
        ...
        // code here
        // 只要能获取连续长度为目标段限长的空间即可

        while (true) {
            if (MEM_SIZE_B - occupiedSize() >= len) {
                break;
            }
            removeSegByLRU();
        }
    ...
    public String page_load(int segIndex, int pageNO) {
        ...
        int len = TLB.TLB_SIZE_B / TLB.LINE_SIZE_B / 2;
        String addr = t.intToBinary(String.valueOf(pageNO * Memory.PAGE_SIZE_B));
        char [] dataRead = this.disk.read(addr, len * Memory.PAGE_SIZE_B); // pageNO~pageNO + len - 1
        char [] pageData = new char[Memory.PAGE_SIZE_B];
        int curPageNO = pageNO;
        SegDescriptor segDescriptor = segTbl.get(segIndex);
        int startFrame = Integer.parseInt(t.binaryToInt("0" + String.valueOf(segDescriptor.getBase()))) / Memory.PAGE_SIZE_B;
        int endFrame = (Integer.parseInt(t.binaryToInt("0" + String.valueOf(segDescriptor.getBase()))) +
                Integer.parseInt(t.binaryToInt("0" + String.valueOf(segDescriptor.getLimit())))) / Memory.PAGE_SIZE_B - 1;
        len = Math.min(len, endFrame - startFrame);
        boolean isLoaded = false;
        int reversedPageNO = -1;
        for (int i = startFrame; i <= endFrame; ++i) {
            if (!reversedPageTbl(i).isValid) {
                isLoaded = true;
                reversedPageTbl(i).vPageNO = pageNO;
                reversedPageTbl(i).updateTimeStamp();
                reversedPageTbl(i).isValid = true;
                reversedPageNO = i;
                break;
            }
        }
        if (!isLoaded) {
            int minIndex = removePageByLRU(startFrame, endFrame);
            reversedPageTbl(minIndex).vPageNO = pageNO;
            reversedPageTbl(minIndex).updateTimeStamp();
            reversedPageTbl(minIndex).isValid = true;
            reversedPageNO = minIndex;
        }
        String pageFrame = t.intToBinary(String.valueOf(reversedPageNO * Memory.PAGE_SIZE_B));
        String res = pageFrame;
        pageTbl(pageNO).setInMem(true);
        pageTbl(pageNO).setFrameAddr(pageFrame.toCharArray());
        int pos = 0;
        for (int i = 0; i <= Memory.PAGE_SIZE_B - 1; ++i) {
            pageData[i] = dataRead[pos++];
        }
        memoryInstance.write(pageFrame, Memory.PAGE_SIZE_B, pageData);
        // add TLB
        tlb.writeTLB(t.intToBinary(String.valueOf(curPageNO)).toCharArray(), pageFrame.toCharArray());
        for (int i = 1; i <= len - 1; ++i) {
            curPageNO += 1;
            for (int j = 0; j <= reversedPageTbl.length - 1; ++j) {
                if (!reversedPageTbl(j).isValid) {
                    pageFrame = t.intToBinary(String.valueOf(j));
                    /*for (int k = 0; k <= Memory.PAGE_SIZE_B - 1; ++k) {
                        pageData[k] = dataRead[pos++];
                    }*/
                    pageData = Arrays.copyOfRange(dataRead, (curPageNO-pageNO)*Memory.PAGE_SIZE_B, (curPageNO + 1 - pageNO)*Memory.PAGE_SIZE_B);
                    tlb.writeTLB(t.intToBinary(String.valueOf(curPageNO)).toCharArray(), pageFrame.toCharArray());///////
                    pageTbl(curPageNO).setInMem(true);
                    pageTbl(curPageNO).setFrameAddr(pageFrame.toCharArray());
                    reversedPageTbl(j).isValid = true;
                    reversedPageTbl(j).updateTimeStamp();
                    reversedPageTbl(j).vPageNO = curPageNO;
                    memoryInstance.write(pageFrame, Memory.PAGE_SIZE_B, pageData);
                    break;
                }
            }
    ...
    private int removePageByLRU(int startFrame, int endFrame) {
        ...
        int tlbNO = tlb.isMatch(reversedPageTbl(minIndex).vPageNO);  //
        if (tlbNO != -1) {
            tlb.invalid(tlbNO);
        }
        return minIndex;
...
//        System.out.println("Start Fit:" + occupiedSegs.size());
    ...
    private class PageItem {
        ...
        private char[] frameAddr;  // 20-bits
