...
public class WriteCacheTest {
    ...
    public void test02(){
        ...
        //char [] dataRead = memory.read(eip3, input2.length);
        //System.out.println(String.valueOf(dataRead));
        //System.out.println(String.valueOf(input2));
        //assertTrue(Arrays.equals(dataRead, input2));
    ...
    public void test03(){
        ...
        assertTrue(Arrays.equals(input2, cache.read(eip2, input2.length))); // read 时需要写回
    ...
    public void TheAnswer() {
        ...
        cache.write(eip, eip2.length(), eip2.toCharArray());
        memory.write(eip2, answer.length(), answer.toCharArray());
        char [] Addr = cache.read(eip, eip.length());  // 地址是32位的
        int len = 32;
        int num = Addr.length / len;
        for (int i = 0; i <= num - 1; ++i) {
            String InfoAddr = String.valueOf(getNthAddr(i, Addr));
            char [] dataWrite = getNthInfo(InfoAddr, len);
            cache.write(eip, len, dataWrite);
        }
        boolean t = true;
        char[] d = cache.read(eip2, answer.length());
        for(int i = 0;i<real.length();i++){
    ...
    public char [] getNthAddr(int n, char [] Addr) {
        if ((n + 1) > Addr.length / 32) {
            return null;
        } else {
            int from = n * 32;
            int to = from + 31;
            char [] resAddr = new char[32];
            for (int i = from; i <= to; ++i) {
                resAddr[i - from] = Addr[i];
            }
            return resAddr;
        }
    }

    public char [] getNthInfo(String eip, int len) {
        char [] Info = memory.read(eip, len);
        return Info;
    }
